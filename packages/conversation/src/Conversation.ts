...
export class Conversation {
  ...
  async generateStream(params: GenerateStreamParams): Promise<StreamResult> {
    ...
    const result = streamText({
      ...
      providerOptions,
      prepareStep:
        pendingImageInjections || this.params.toolImageRetention != null || provider === 'anthropic'
          ? ({ messages: stepMessages }) => {
              ...
              if (provider === 'anthropic') {
                // Runs for every step including the first, so this is the
                // single seam where outgoing Anthropic requests get cache
                // breakpoints (after pruning — marks must land on the final
                // per-step messages).
