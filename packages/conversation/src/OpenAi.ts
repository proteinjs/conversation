...
export class OpenAi {
  static async generateResponse(
    ...
    if (messageModerators) messageParamsWithHistory = OpenAi.moderateHistory(messageParamsWithHistory, messageModerators);
  ...
  private static moderateHistory(history: MessageHistory, messageModerators: MessageModerator[]) {
    for (const messageModerator of messageModerators) history.setMessages(messageModerator.observe(history.getMessages()));
  ...
  private static async executeRequest(messageParamsWithHistory: MessageHistory, logLevel: LogLevel, functions?: Function[], model?: string): Promise<ChatCompletion> {
    ...
    try {
      ...
      else if (latestMessage.role == 'function') logger.info(`Sending request: returning output of ${latestMessage.name} function`);
      ...
      else if (responseMessage.function_call) logger.info(`Received response: call ${responseMessage.function_call.name} function`);
  ...
  static async generateCode(
    ...
    const resolvedHistory = history ? (includeSystemMessages ? history.push(systemMessages) : history) : includeSystemMessages ? new MessageHistory().push(systemMessages) : undefined;
