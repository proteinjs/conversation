 ...
 * https://www.recraft.ai/docs/api-reference/usage as read on 2026-09-22: base
 * `https://external.api.recraft.ai/v1`, `Authorization: Bearer`; `POST /images/generations`
 * (`prompt`, `model`, `size` as `WxH`, `n` 1–6, `response_format: b64_json`, `negative_prompt`)
 * answering `{ data: [{ b64_json }], created }` — a `_vector` model's `b64_json` is SVG text;
 * `POST /images/generations/vector` is the same door with raster models refused by the server, so
 * a vector model is always sent through it; `POST /images/vectorize` and
 * `POST /images/removeBackground` take one picture as multipart `file` (PNG, JPG, WEBP under
 * 10 MB, at most 16 MP and 4096 px an edge, at least 256 px) with `response_format: b64_json` and
 * answer `{ image: { b64_json } }`.
 *
...
export class RecraftImageAdapter implements ImageProviderAdapter {
  ...
  private buildRequest(
    ...
    if (operation === 'generate') {
      ...
      const door = RecraftImageAdapter.isVectorModel(request.model)
        ? 'images/generations/vector'
        : 'images/generations';
      return { url: `${this.baseUrl}/${door}`, headers, body: { kind: 'json', json }, signal };
    }
  ...
  private readAnswer(request: ImageGenerationRequest, response: ImageTransportResponse): ImageAdapterResult {
    ...
    const mimeType = svg ? RECRAFT_SVG_MIME_TYPE : `image/${request.outputFormat ?? 'png'}`;
